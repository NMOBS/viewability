...
export const check = (element, options) => {
  ...
  if (options.window.screenX < (browserLeftOffset * -1)) { windowLeftOffScreen = Math.abs(options.window.screenX + browserLeftOffset) }
  let windowTopOffScreen = 0
  if (options.window.screenY < (browserTopOffset * -1)) { windowTopOffScreen = Math.abs(options.window.screenY + browserTopOffset) }
